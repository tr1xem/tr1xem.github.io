...
export default function handleTyping(
	...
	instance
		...
		.type(
			'<span style="color: #118c0a;">trix@Life</span>: <span style="color: #2c5bc7;">/</span> $ ',
		...
		.type("cat life")
		.break({ instant: true })
		.type("cat: life: No such file or directory", { instant: true })
		.break({ instant: true })
		.type(
			'<span style="color: #118c0a;">trix@Life</span>: <span style="color: #2c5bc7;">/</span> $ ',
		...
		.break({ instant: true })
		.type(
			'<span style="color: #118c0a;">trix@Life</span>: <span style="color: #2c5bc7;">/</span> $ ^C',
		...
		.break({ instant: true })
		.type(
			"The authenticity of host 'trix.is-a.dev (173.208.244.6)' can't be established.",
			{ instant: true },
		)
		.type(
		...
		.break({ instant: true })
		.type(
			"Warning: Permanently added 'trix.is-a.dev,173.208.244.6' (ECDSA) to the list of known hosts.",
			{ instant: true },
		)
		.break({ instant: true })
		...
		.type(
			"Get:1 https://trix.is-a.dev/download googletagmanager font-inter trix-website highlightjs highlightjs-atom-one-theme jbox jquery analytics typeit [548 kB]",
			{ instant: true },
		)
		.break({ instant: true, delay: 560 })
		.type("Fetched 548 kB in 1s (1972 kB/s)", { instant: true })
		.break({ instant: true, delay: 300 })
		.type("Selecting previously unselected package trix-website.", {
			instant: true,
		})
		.break({ instant: true })
		...
		.type("Preparing to unpack .../trix-website_latest.deb ...", {
			instant: true,
		})
		.break({ instant: true })
